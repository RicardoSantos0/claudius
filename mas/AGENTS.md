...
- `mas/AGENTS.md` is the canonical MAS instruction source for every provider
  surface; `mas/CLAUDE.md` is only a Claude Code import shim.
- Claude, Codex, OpenCode, GitHub Copilot, and other client-local memories are
  non-authoritative recall caches. At project close, `mas close` preserves or
  creates `PROJECT_SUMMARY.md` and embeds the same text in the `project_closed`
  SQL event. Every provider retrieves that shared summary through the normal
  MAS prompt/MCP path instead of synchronizing private client formats.
- Before `mas close`, durable provider-local learnings must be recorded through
  the normal handoff/shared-state flow or a curated `PROJECT_SUMMARY.md`.
...
### Mode 1 — Manual provider surfaces (no API credits needed) Primary

Invoke `master_orchestrator` from Claude Code, Codex, OpenCode, GitHub Copilot,
or another compatible client. Use `mas prompt` to assemble the next agent
prompt, then run the agent in the selected surface; no provider API key is
required for the manual loop.

The Python engine handles all state, handoffs, governance, and shared memory.
The selected manual surface supplies model execution and returns the response.
...
mas prompt  <project-id> [agent-id]   # Assemble agent prompt (manual surface mode)
...
## Manual Surface Mode — Governance Checklist

When running projects through any manual surface (no API credits), enforce these
...
**Note:** `handoff_quality` scores 0 in manual mode (no rejection cycle).
...
└── AGENTS.md           ← canonical MAS instructions
```
